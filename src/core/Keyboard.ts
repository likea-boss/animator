  ...
  private handleKeyPress(event: KeyboardEvent) {
    ...
    handlersToCall.forEach((h: IKeyboardHandler) => {
      try {
        ...
        EventEmitter.emit(Keyboard.KEYBOARD_HANDLER_CALL_ERROR, {}) // TODO добавить логеру метод для вывод ошибок
      }
    })
  }

  addHandler(handler: () => void, options: IKeyboardOptions) {
    this.handlers.push({ handler, options });
  }

  setAliases() { // TODO добавить алиасы для мета клавиш по типу alt option равны и при нажатии на alt вызываються все методы которые забинжены на option и наоборот
